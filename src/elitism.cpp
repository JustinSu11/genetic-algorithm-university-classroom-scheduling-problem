...
std::vector<ChromosomeStruct> elite(std::vector<ChromosomeStruct> &chromofitness) {
 std::vector<ChromosomeStruct> theBest; // vector to hold the best of the best
 int numofelite = 4; // number of elites that get to move to the elite array
 int count = 0;
...
for(int i = 0 + 1; i < chromofitness.size(); i++){
 if(largest < chromofitness[i].fitness){
  ...
  theBest.push_back(chromofitness[index]); // put largest into theBest vector
